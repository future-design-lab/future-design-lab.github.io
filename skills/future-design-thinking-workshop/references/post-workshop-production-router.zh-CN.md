...
进入后期后，AI 不只是被动等待学生说“做个 PPT”。它需要像项目制作导师一样，主动帮助学生判断什么呈现形式最适合他们的方案，并鼓励学生把成果做完整、做丰富、做成一个可以被评审理解和体验的项目包。

AI 每次进入后期都要先做三件事：

1. 读完前期工作坊产物，确认方案的核心逻辑、目标用户、关键场景和实施路径。
2. 根据方案类型推荐 2-4 种合适的呈现形式，而不是只推荐一种。
3. 建立或更新本地项目档案文件夹，把方法过程、素材、生成资产和最终交付物结构化保存。

引导语可以这样说：

```text
你的方案已经完成了前期逻辑。接下来我们不要只做一份 PPT，而是把它做成一个完整的项目呈现包。

我建议你至少选择：
1. 一个“讲清楚逻辑”的产物：项目文档 / 答辩 PPT；
2. 一个“让人看见未来”的产物：场景效果图 / 明日头条 / 海报；
3. 一个“让人体验机制”的产物：交互原型 / 服务蓝图 / 故事板；
4. 如果时间允许，再加一个“传播型产物”：宣传视频 / 未来新闻 / 网站页面。

我会帮你把每一步生成的内容都存到本地项目文件夹里，最后形成一个清晰的项目档案。
```
...
每个学生都应该在本地有一个完整项目文件夹。AI 在第一次进入后期制作时，应建议或自动创建以下结构。文件夹名建议使用英文或拼音，避免空格和特殊字符。

推荐根目录：

```text
future-design-projects/
└── <student-name-or-team>-<project-slug>/
```
...
│   ├── interpretation.md
│   ├── provotyping-card.md
│   ├── tomorrow-headline.md
│   └── backcasting-roadmap.md
...
保存后期制作的统一输入包。每次方案逻辑发生变化，都先更新这个文件，再继续生成 PPT、文档、图片或视频。
...
```text
| 日期 | 资产名称 | 文件路径 | 生成/来源 | 用途 | 对应方案内容 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-30 | future-classroom-hero.png | 03_visual-assets/generated-images/future-classroom-hero.png | imagegen | PPT封面/报告主图 | 2040 AI 学科课堂 | selected |
```

### `prompts.md`

记录关键提示词和修改方向，尤其是图片、视频、原型、PPT 的生成提示词。这样学生后续可以复现、修改或解释 AI 共创过程。

归档规则：
- 正式交付物放在 `08_delivery/final-submission/`。
- 可编辑源文件保留在对应制作目录，例如 `.pptx` 在 `06_presentations/`，`.docx` 在 `05_documents/`。
- 生成图片和参考图片分开放，避免混淆来源。
- 每次生成重要资产后，更新 `asset-index.md`。
- 不覆盖旧版本；需要迭代时使用 `v01`、`v02`、`final` 等后缀。
- 如果某个文件用于最终提交，必须在 `08_delivery/links.md` 或 `final-submission/README.md` 中说明。

## 3. 呈现形式推荐逻辑

AI 需要根据方案类型主动建议组合，而不是只问“你要做什么”。推荐逻辑如下：

| 方案类型 | 建议呈现组合 | 为什么 |
| --- | --- | --- |
| App / 平台 / 数字工具 | PPT + 交互原型 + UI效果图 + 用户流程图 | 评审需要看清功能、流程和体验 |
| 公共服务 / 城市系统 / 教育系统 | 项目文档 + 服务蓝图 + 利益相关者图 + Backcasting 路线图 + 场景图 | 重点是系统关系、政策路径和多方协作 |
| 产品 / 装置 / 空间体验 | 效果图 + 使用故事板 + 图纸/结构说明 + 展示海报 + PPT | 重点是看见形态、理解使用方式 |
| 社会创新 / 社区项目 | 项目文档 + 用户旅程 + 宣传海报 + 行动路线表 + 访谈/故事材料 | 重点是人群、触点、组织方式和落地性 |
| 未来叙事 / 未来新闻 / 传播项目 | 明日头条图 + 故事板 + 宣传视频 + 网站页面 + PPT | 重点是情绪感染力和未来可感知性 |
| 策略 / 政策 / 教育课程 | 白皮书/报告 + 路线图 + 评价指标表 + 课程/服务原型 + PPT | 重点是逻辑、实施路径和评估方法 |

AI 推荐时要给出“最低组合”和“加分组合”：

```text
最低组合：项目报告 + 答辩 PPT + 2 张关键效果图。
加分组合：再加入服务蓝图、交互原型和 60 秒概念视频。
```
...
4. 每一步会生成什么文件；
5. 做完后如何检查质量；
6. 请在本地为我建立清晰的项目文件夹，并把生成的内容按资产类型归档。
```
...
```text
方案逻辑是否完整：是否能从未来信号推到挑战、洞察、方案和行动路线？
呈现形式是否丰富：是否同时有文字说明、视觉呈现和体验/流程呈现？
关键资产是否齐全：PPT、报告、图片、原型、视频、图表是否在本地项目文件夹中？
文件是否可编辑：是否保留 .pptx、.docx、源图、原型源文件？
最终提交是否清楚：08_delivery/final-submission/ 是否包含最终提交文件和链接说明？
AI 共创过程是否可追踪：prompts.md、asset-index.md、version-history.md 是否更新？
```

AI 在最终回复中不要只说“完成了”，而要给学生一个项目资产清单：
