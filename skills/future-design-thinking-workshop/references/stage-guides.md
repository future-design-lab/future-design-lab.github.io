...
- **Generate it with the `generate-multimodal-media` skill** (do not just describe it, and do not leave a text placeholder):
  ...
  2. **Remind the user to grant the agent full access / file-write permission before generating** (Codex: full-access mode / approve file writes; Claude Code: allow write to the working folder; other agents: enable "full access" / file write). Saving the image and updating the state file are disk writes — in a restricted/ask-every-time mode they get blocked, so the image never reaches the page and the canvas silently stays empty.
  3. Once confirmed, **call `generate-multimodal-media`** (image model `gpt-image-2` via `POST /images/generations` at `https://llm.mantai.me/v1`; the skill handles `MANTA_API_KEY`). If that skill is not yet installed, install it from `assets/downloads/generate-multimodal-media.zip` first.
  4. **Write the image prompt as a layout/composition brief, not just a scene.** Spell out the media format and its layout regions, name each text block and quote its exact wording so the model renders it as legible typography, place the masthead/site/platform name, the photo area + caption, date/year, and a news/launch mood. Explicitly state that the headline and body text must be rendered as clear, correctly-composed type within the layout. Use a layout-appropriate aspect ratio (e.g. tall portrait `size` like 1024x1536 for a newspaper front page or poster; wide `size` like 1536x1024 for a website hero or big screen; near-square for a 微博 card).
  5. **Sync it to the canvas:** save the returned image into the **same working folder as `future-design-canvas.html`** (e.g. `headline-2040.png`), set `headline.image` to that filename in `future-design-canvas-state.json`, and write the state file so the canvas (which polls ~every 1.2s) renders it. The image must sit next to the HTML because the canvas loads it by relative path.
  6. Confirm the canvas actually shows the headline image, then check the typography with the user: are the headline and key text legible and correctly spelled? If the layout is wrong, text is garbled, the image didn't appear (often a permission/path issue), or it reads as a plain photo, fix and regenerate before locking the stage.
